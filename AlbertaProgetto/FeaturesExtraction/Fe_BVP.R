...
getFeatures_BVP<-function(baseline_values, interval_values){
  ...
  #BVP_preprocessed <- interval_values
  #baseline_preprocessed <- baseline_values
  
  #print(paste("baseline preprocessed", length(baseline_preprocessed)))
  peaks_baseline <- AMPD(baseline_preprocessed, extended = TRUE)
  # print(paste("BVP preprocessed", length(BVP_preprocessed)))
  peaks_interval <- AMPD(BVP_preprocessed, extended = TRUE)
  # print(paste("BVP normalized", length(BVP_normalized)))
  ...
  BVP_features <-c(difference_BVPpeaks_ampl, mean_BVPpeaks_ampl, min_BVPpeaks_ampl, max_BVPpeaks_ampl, sum_peak_ampl)
  return(BVP_features)
  
}

getFeatures_BVP_baseline<-function(baseline_values){
 ...
 # difference_BVPpeaks_ampl <- getDifferenceMeanBVPPeakAmplitude(baseline_preprocessed, peaks_baseline, BVP_preprocessed, peaks_interval ) 
  mean_BVPpeaks_ampl <- getMeanBVPPeakAmplitude(baseline_preprocessed, peaks_baseline)
  min_BVPpeaks_ampl <- getMinBVPPeakAmplitude(baseline_preprocessed,peaks_baseline)
  max_BVPpeaks_ampl <- getMaxBVPPeakAmplitude(baseline_preprocessed,peaks_baseline)
  sum_peak_ampl <- getSumPeakAmplitude(baseline_preprocessed,peaks_baseline)
